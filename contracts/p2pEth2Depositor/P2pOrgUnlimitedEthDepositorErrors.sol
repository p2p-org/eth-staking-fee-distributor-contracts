...
error P2pOrgUnlimitedEthDepositor__FailedToSendEth(
    address _receiver,
    uint256 _amount
);
...
error P2pOrgUnlimitedEthDepositor__CallerNotClient(
    address _caller,
    address _client
);
...
error P2pOrgUnlimitedEthDepositor__WaitForExpiration(
    uint40 _expiration,
    uint40 _now
);
...
error P2pOrgUnlimitedEthDepositor__CallerNotEip7251Enabler(
    address _caller,
    address _eip7251Enabler
);
...
error P2pOrgUnlimitedEthDepositor__EthAmountPerValidatorInWeiOutOfRange(
    uint256 _ethAmountPerValidatorInWei
);

/// @notice Withdrawal credentials prefix must be either 0x01 or 0x02
error P2pOrgUnlimitedEthDepositor__IncorrectWithdrawalCredentialsPrefix(
    bytes1 _passedPrefix
);

/// @notice Withdrawal credentials bytes 2 - 12 must be zero
error P2pOrgUnlimitedEthDepositor__WithdrawalCredentialsBytesNotZero(
