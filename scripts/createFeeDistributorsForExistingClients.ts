...
const clientConfigs = [
    ...
    { recipient: '0x0bc9bad40e6F1eE8e472A4E434a615DC3aD30B60', basisPoints: 9000 },
...
async function main() {
    try {
        ...
        for (let i = 0; i < clientConfigs.length; i++) {
            ...
            const validatorData = {
                firstValidatorId: validatorDataArray[i].oracleId,
                validatorCount: validatorDataArray[i].validatorCount,
                clientOnlyClRewards: ethers.BigNumber.from(validatorDataArray[i].sum).mul(1e9)
            }
